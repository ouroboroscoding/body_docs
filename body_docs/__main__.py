...
# Local impors
from . import EXTENSIONS, handle_file, generate_service
...
def cli() -> int:
	...
	oArgParser.add_argument(
		...
		help='The format to out the documentation in. Supports: %s' %
	...
	# Split format by comma
	oArgs.format = oArgs.format.split(',')

	# If any format isn't valid
	for sFormat in oArgs.format:
		if sFormat not in EXTENSIONS:
			print('Invalid option for --format: "%s"' % sFormat,
				file = stderr)
			print('body-docs supports %s' % ', '.join(EXTENSIONS.keys()))
			return 0
	...
	if lServices:
		...
		for dService in lServices:
			for sFormat in oArgs.format:
				print('Generating %s for "%s"' % ( sFormat, dService['name']) )
